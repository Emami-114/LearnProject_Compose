...
fun TopBar(
    ...
    TopAppBar(title = {
        Row(modifier = Modifier.fillMaxWidth(), verticalAlignment = Alignment.CenterVertically) {
            Text(text = title, color = TextWhite, style = androidx.compose.material.MaterialTheme.typography.h5, fontWeight = FontWeight.Medium)
            Row(modifier = Modifier.fillMaxWidth(),horizontalArrangement = Arrangement.End) {
                IconButton(onClick = { openMenu = !openMenu }) {
                    Icon(imageVector = Icons.Outlined.MoreVert, contentDescription = null, tint = TextWhite,
        ...
        actions = {
        DropdownMenu(modifier = Modifier.background(TextWhite),
            expanded = openMenu, onDismissRequest = { openMenu = !openMenu }) {
